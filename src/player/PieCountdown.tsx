...
const calcArc: (...args: any) => D = (pFrom: number, pTo: number, r: number = 1, cx: number = 0, cy: number = 0) => {
...
const Arc = (props: ArcProps) => {
  ...
  const {startX, startY, endX, endY, largeArcFlag} = useMemo(() => calcArc(from, to, r, cx, cy), [from, to, r, cx, cy]);
  const d = 
    `M ${startX} ${startY} ` +
